...
# 2. device
device_ids = [0,1,2,3]     # 사용할 Device ID 설정
device = torch.device('cuda:{}'.format(min(device_ids)) if torch.cuda.is_available() else 'cpu')


def parse(args):
    parser = argparse.ArgumentParser()
    parser.add_argument('--dist_mode', type=str, default='ddp', help='dp or ddp or none')            # DP 인지 DDP 인지 설정
    parser.add_argument('--port', type=str, default='8900')
    ...
    parser.add_argument('--save_file_name', type=str, default='detr_coco_exp3')  # FIXME
    ...
    opts.gpu_id_min = min(device_ids)       # device id의 맨 첫번째
    print('use device ids : {}'.format(device_ids))
    print(opts)
    return opts
